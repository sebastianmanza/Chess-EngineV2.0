...
public class GameState {
    ...
    private boolean promotionRow(int square) {
        if (this.turnColor) {
            return (square > 56);
        } else return (square < 8);
    }

    private boolean startingRow(int square) {
        if (this.turnColor) {
            return (square < 16);
        } else return (square > 48);
    }
    /**
    ...
    public short[] nextMoves() {
        ...
        while(pawns != 0) {
            ...
            /* If there was nothing in the way before, and the pawn is on it's starting row, add the start possibility. */
            if (attackBoard != 0 && startingRow(pawnSquare)) {
                attackBoard |= pawnStarts[pawnSquare] & ~bitBoards[ALLPIECES];
            } //if
            
            /* Join the 'quiet' moves with the captures. To capture, there must be a piece from the opponent in that slot. */
